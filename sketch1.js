...
function init() {
	...
	var paths = MORPH.LoadSVG(['path1.svg', 'path2.svg'], function (paths) {
		morph = new MORPH.Morph(paths, {looping:true, duration:3000}).start();
	});
...
function loop() {
	...
	if (morph) {
		...
		for (var i = 0; i < segmentCollection.length; i++) {
			...
			b.ctx.bezierCurveTo(segment.ctrl1.x, segment.ctrl1.y, segment.ctrl2.x, segment.ctrl2.y, segment.pt2.x, segment.pt2.y);
		}

		b.ctx.stroke();
	}

	MORPH.update(Date.now());
